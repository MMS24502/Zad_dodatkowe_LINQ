...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        public static IEnumerable<object> Task10()
        {
            IEnumerable<object> result = Emps
                ...
                .Union(new[] { new { Ename = "Brak wartości", Job = (string)null, HireDate = (DateTime?)null } }) // Create a single-element array with a compatible anonymous type
                .ToList();
            return result;
        }
        ...
        public static IEnumerable<object> Task11()
        {
            IEnumerable<object> result = Emps
                ...
                .Select(group => new { name = Depts.FirstOrDefault(dept => dept.Deptno == group.Key)?.Dname, numOfEmployees = group.Count()})
                .ToList();
            return result;
        }
        ...
        {
            int result = arr
                .GroupBy(x => x)                       // Group numbers by their value
                .Where(g => g.Count() % 2 != 0)        // Filter groups where the count is odd
                .Select(g => g.Key)                    // Select the key of the group (the number itself)
                .First();    
            return result;
        }
        ...
        {
            var deptsWithEmpCounts = Emps
                .GroupBy(emp => emp.Deptno)
                .Select(group => new { Deptno = group.Key, Count = group.Count() })
                .Where(x => x.Count is 5 or 0)
                .ToList();
            
            IEnumerable<Dept> result = Depts
                .Where(dept => deptsWithEmpCounts.Any(d => d.Deptno == dept.Deptno))
        ...
        {
            var relevantDeptNos = Emps
                .Where(emp => emp.Job.Contains("A"))                 
                .GroupBy(emp => new { emp.Deptno, emp.Job })         
                .Where(g => g.Count() > 2)                           
                .Select(g => g.Key.Deptno)                           
                .Distinct();                                         
            
            var result = Depts
                .Where(dept => relevantDeptNos.Contains(dept.Deptno))
                .OrderBy(dept => dept.Dname);   
            return result;
        }
    ...
    public static class CustomExtensionMethods
    {
        ...
        {
            var result = emps
                .Where(manager => emps.Any(subordinate => subordinate.Mgr != null && subordinate.Mgr.Empno == manager.Empno))
                .OrderBy(e => e.Ename)
        ...
        public static IEnumerable<int?> GetRelevantDept(IEnumerable<Emp> emps)
        {
            IEnumerable<int?> relevantDept = emps
                .Where(emp => emp.Job.Contains("A"))                 
                .GroupBy(emp => new { emp.Deptno, emp.Job })         
                .Where(g => g.Count() > 2)                           
                .Select(g => g.Key.Deptno)                           
